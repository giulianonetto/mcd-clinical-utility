...
compute_nb_untreated <- function(bayesdca_fit, decision_strategy = "mced_test") {
    nb_mced <- bayesdca_fit$fit$distributions$net_benefit[[decision_strategy]]
    nb_ta <- bayesdca_fit$fit$distributions$treat_all
    prev <- bayesdca_fit$fit$distributions$prevalence

    delta_ut <- treat_none_ut <- nb_mced_ut <- matrix(
        nrow = nrow(nb_mced),
        ncol = ncol(nb_mced)
    )
    for (i in seq_along(bayesdca_fit$thresholds)) {
        .t <- bayesdca_fit$thresholds[i]
        w_t <- .t / (1 - .t)
...
plot_net_benefit_opt_out <- function(bdca_fit, .title = "Net benefit (opt-out)", .colors = NULL) {
    df <- compute_nb_untreated(bdca_fit)
    ...
    df %>%
        ...
        ggplot2::geom_ribbon(alpha = 0.5, fill = .colors["MCED test"]) +
        ggplot2::geom_line(linewidth = 1.2, ggplot2::aes(color = "MCED test")) +
        ggplot2::geom_ribbon(
            alpha = 0.5,
            ggplot2::aes(
                x = thr,
